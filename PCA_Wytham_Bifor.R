...
merged$cum_ozone<-as.double(merged$cum_ozone)
#PCA by prcomp
PCA<-prcomp(merged)
...
legend(2000,0.105,legend = c("Component 1","Component 2","Component 3","Component 4"), col = c("black","#f35e5a","#f1c40f", "#17b3b7"), lwd =2, cex=0.8, bty="n")
...
labsdf$CUM_OZ<-c(merged$cum_ozone)
...
PC12<-fviz_pca_ind(pca, geom.ind = "point", axes=c(1,2),
                   ...
                   legend.title = "Site")+ggtitle("")+theme_cowplot(12)+
...
oz<-fviz_pca_ind(pca, geom.ind = "point",axes = c(1,2),
             col.ind = labsdf$OZ,# color by OZONE
             palette = 'all',
             addEllipses = FALSE, ellipse.type = "convex",
             legend.title = "Ozone")+ggtitle("")+scale_color_continuous(type = "viridis")+theme_cowplot(12)+
  theme(text = element_text(family = "Helvetica"))
oz

cum_oz<-fviz_pca_ind(pca, geom.ind = "point",axes = c(1,2),
                 col.ind = labsdf$cum_ozone,# color by cumulative OZONE
                 palette = 'all',
                 addEllipses = FALSE, ellipse.type = "convex",
                 legend.title = "Cumulative\nOzone (ppb)")+ggtitle("")+scale_color_continuous(type = "viridis")+theme_cowplot(12)+
  theme(text = element_text(family = "Helvetica"))
cum_oz
setwd("~/Library/CloudStorage/OneDrive-Nexus365/01 OXFORD PHD/Field Work 2023/R_Data_Analysis/Wytham_Bifor_Ozone")
tiff("F3_PCA.tiff", units="in", width=14, height=6, res=300)
plot_grid(loadingsplot,plot_grid(PC12, PC23, PC13,cum_oz, labels = c('B','C','D','E')), labels=c('A',''), label_size = 16)
dev.off()
