...
VIdf<-data.frame(merged$site, merged$date, merged$site_date ,merged$ozone,merged$OZDI,merged$NDVI,merged$MCARI, merged$PRI, merged$PSRI)#needed for ggplots
colnames(VIdf)<-c("site", "date", "site_date","ozone","OZDI","NDVI","MCARI","PRI","PSRI")
...
means_VI<-means_VI[-5]
means_VI<-means_VI[-3]
colnames(means_VI)<-c("site_date", "site", "date",  "ozone", "OZDI" ,"NDVI", "MCARI", "PRI", "PSRI" )
